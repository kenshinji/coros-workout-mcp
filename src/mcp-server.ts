...
import type { CatalogExercise, Region, RunStep } from "./types.js";

export interface CreateServerOptions {
  /**
   * Persist a rebuilt exercise catalog and refresh the in-memory copy, then
   * return a human-readable description of where it went. Omit it and the
   * `update_exercises` tool is not registered — there is nowhere to put the
   * result.
   */
  saveCatalog?: (catalog: CatalogExercise[]) => Promise<string>;
}

/**
 * Build the MCP server with every tool registered. Both entry points use this:
 * `index.ts` over stdio, `worker.ts` over Streamable HTTP.
 */
export function createServer(options: CreateServerOptions = {}): McpServer {
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Authenticated successfully. User ID: ${auth.userId}, Region: ${auth.region}. Token stored for subsequent calls.`,
            },
          ],
        };
      } catch (error) {
          ...
          content: [
            {
              ...
              text: [
                ...
                workoutId
                  ? `Workout id: ${workoutId} (pass it to schedule_workout to put it on a date)`
                  : `Workout id: not returned by COROS — use list_workouts to find it`,
  ...
  // Only offered when the host can store the rebuilt catalog somewhere.
  const saveCatalog = options.saveCatalog;
  if (saveCatalog) {
    server.tool(
      "update_exercises",
      ...
      async ({ sportType }) => {
        try {
          ...
          // Persist however the host can (a file on disk, KV in a Worker)
          const location = await saveCatalog(catalog);
          ...
          lines.push(`Catalog written to: ${location}`);
  ...
  server.tool(
    ...
    async ({ name, overview, steps }) => {
      try {
        ...
        const workoutId = await addRunWorkout(auth, name, overview, exercises, calculated);
  ...
  server.tool(
    "schedule_workout",
    "Put an existing COROS workout on a calendar date, so it appears on that day in Training Hub and on the watch. Identify it by workoutId (returned by create_workout/create_run_workout and list_workouts) or by workoutName. Scheduling one day does not affect other days.",
    {
      workoutId: z
        .string()
        .optional()
        .describe("Workout id, as returned by create_workout/create_run_workout/list_workouts (e.g. '1234567890123456789')"),
      workoutName: z
        .string()
        .optional()
        .describe("Workout name, used when the id isn't known — the newest workout with this name is scheduled"),
    ...
    async ({ workoutId, workoutName, date, sortNoInSchedule }) => {
      try {
        const auth = await getValidAuth();
        if (!auth) {
          return {
        ...
        const id = workoutId ?? (workoutName ? await findWorkoutIdByName(auth, workoutName) : null);
        if (!id) {
          return {
            content: [
              {
                type: "text" as const,
                text: workoutName
                  ? `No workout named "${workoutName}" found. Use list_workouts to see what exists.`
                  : "Pass either workoutId or workoutName.",
              },
            ],
            isError: true,
          };
        }
        const res = await scheduleWorkout(auth, id, date, sortNoInSchedule);
  ...
  server.tool(
    ...
    "List workouts scheduled on the COROS calendar between two dates. The ids shown are the plan's own copies, not library workout ids — use list_workouts for an id to pass to schedule_workout.",
    {
      ...
      try {
        ...
        const rows = (plan.entities ?? [])
          ...
          .map((e) => {
            ...
            // Not the library id: scheduling copies the workout into the plan
            // document, and this is the copy's id. schedule_workout needs the
            // id from list_workouts.
            return `  ${when}: ${p.name} (scheduled copy ${p.id})${km}${mins}`;
          });
      ...
      try {
        ...
        const formatted = workouts
          .map((w) => {
            ...
            const sport = w.sportType === 1 ? "run" : w.sportType === 4 ? "strength" : `sportType ${w.sportType}`;
            return `- **${w.name}** — id ${w.id} (${sport}, ${durationMin} min, ${w.totalSets || 0} sets, ${w.exerciseNum || 0} exercises)${w.overview ? `\n  ${w.overview}` : ""}`;
          })
          .join("\n");
